# Clean Entity Registry Data
# Alexander Nielson alexnielson@utah.gov

# This script cleans both Entity Registry Data and OSA Salesforce 
# entity name data. 
...
sf <-  sf_auth(username = "email_here",
               password = 'pass_here',
...
#' impute_entity_name
#' In the dataset sent to our office by the LG's office, there is a gap in the 
#' entity name for board members and other contacts. This script allows us to 
#' impute the entity name above a given row by saving the current index of the 
#' entity name as a global string which is allocated if the entity name is empty
#' @param entity_name_p 
#'
#' @return
#' @export
...
standardize_name <- function(name_str){
  
  
  name_str %>% 
    #Fix common special characters
    fifelse( stri_detect_regex(.,"&"),  stri_replace_all_regex(.,"&", " and "),.) %>% 
    fifelse( stri_detect_regex(.,"@"),  stri_replace_all_regex(.,"@", "at"),.) %>% 
    ...
    fifelse( stri_detect_regex(.,"ST\\sGEORGE"),  stri_replace_all_regex(.,"ST\\sGEORGE", "Saint George"),.) %>%
    fifelse( stri_detect_regex(.,"ST\\. GEORGE"),  stri_replace_all_regex(.,"ST\\. GEORGE", "Saint George"),.) %>%
    fifelse( stri_detect_regex(.,"St Marks"),  stri_replace_all_regex(.,"St Marks", "Saint Marks"),.) %>% 
    fifelse( stri_detect_regex(.,"ST MARKS"),  stri_replace_all_regex(.,"ST MARKS", "Saint Marks"),.) %>% 
    fifelse( stri_detect_regex(.,"ST\\. MARKS"),  stri_replace_all_regex(.,"ST\\. MARKS", "Saint Marks"),.) %>% 
    fifelse( stri_detect_regex(.,"Dist$"), stri_replace_all_regex(.,"Dist$","District"),.) %>% 
    ...
    fifelse( stri_detect_regex(.,"Dept of"),  stri_replace_all_regex(.,"Dept of", "Department of "),.) %>% 
    fifelse( stri_detect_regex(.,"DEPT OF"),  stri_replace_all_regex(.,"DEPT OF", "Department of "),.) %>% 
    fifelse( stri_detect_regex(.,"Dept\\. of"),  stri_replace_all_regex(.,"Dept\\. of", "Department of "),.) %>% 
   
    #Handle Inc, LLCs, etc.
    
    stri_replace_all_regex(", Incorporated", "") %>% 
    stri_replace_all_regex(", Inc\\.", "") %>% 
    stri_replace_all_regex(", Inc", "") %>% 
    stri_replace_all_regex(", INC", "") %>% 
    stri_replace_all_regex(" INC.", "") %>% 
    stri_replace_all_regex(", LLC", "") %>% 
    stri_replace_all_regex(", LLP", "") %>% 
    stri_replace_all_regex(" LLC", "") %>% 
    stri_replace_all_regex(" LLP", "") %>% 
    stri_replace_all_regex("Inc\\.", "") %>% 
    stri_replace_all_regex(" Inc$","") %>% 
    ...
    # remove ,
    stri_replace_all_regex(",","") %>% 
    #convert to special title case format
    convert_to_osa_titlecase() %>% 
    # sometimes there is an extra space so remove it
    stri_replace_all_regex("\\s+"," ") %>%
    stri_trim_both()
...
#' convert_to_osa_titlecase
#' This function takes entity names, converts them to title case and replaces 
#' common words that we do not want in title case in the correct case.
#'
#' @param name_str 
#'
#' @return
...
convert_to_osa_titlecase <- function(name_str){
  ...
  name_str %>% 
    ...
    stri_replace_all_regex(" A ", " a ") %>% 
    stri_replace_all_regex(" In ", " in ") %>% 
    ...
    # Doctors/Health Provider
    stri_replace_all_regex(" Mdpc", " MDPC") %>% 
    stri_replace_all_regex(" Md", " MD") %>% 
    stri_replace_all_regex(" Pc", " PC") %>% 
    stri_replace_all_regex(" Pplc", " PPLC") %>% 
    stri_replace_all_regex(" Pllc", " PLLC") %>% 
    stri_replace_all_regex(" Plcc", " PLCC") %>% 
    stri_replace_all_regex(" Lc", " LC") %>% 
    stri_replace_all_regex(" Od", " OD") %>% 
    stri_replace_all_regex(" Dpm", " DPM") %>%
    stri_replace_all_regex(" Do(?![[:alpha:]])", " DO") %>% 
    stri_replace_all_regex(" Np", " NP") %>% 
    stri_replace_all_regex(" Ob-gyn", " OBGYN") %>% 
    stri_replace_all_regex(" Obgyn", " OBGYN") %>% 
    stri_replace_all_regex(" Ob", " OB") %>% 
    stri_replace_all_regex(" Dds", "DDS") %>% 
    stri_replace_all_regex(" Lc", "LC") %>% 
    stri_replace_all_regex(" Dmd", " DMD") %>% 
    stri_replace_all_regex(" Cra(?![[:alpha:]])", " CRA") %>% 
    
    #Entity Specific
    stri_replace_all_regex("Af Cnms", "AF CNMS") %>% 
    stri_replace_all_regex("Arup Lab", "ARUP Lab") %>% 
    stri_replace_all_regex("Bdo Usa", "BDO USA") %>% 
    stri_replace_all_regex("Bjb$", "BJB") %>% 
    stri_replace_all_regex("B2a", "B2A") %>%
    ...
    stri_replace_all_regex("Ucair", "UCAIR") %>% 
    stri_replace_all_regex("Uipa Crossroads Public Infrastructure District", "UIPA Crossroads Public Infrastructure District") %>% 
    ...
    stri_replace_all_regex("Ddi Vantage", "DDI Vantage") %>% 
    stri_replace_all_regex("Cba Center", "CBA Center") %>% 
    stri_replace_all_regex(" Mbr", " MBR") %>% 
    stri_replace_all_regex("Cta Community Supports", "CTA Community Supports") %>% 
    stri_replace_all_regex("Hill Afb Mtf", "Hill Air Force Base MTF") 
...
# there are gaps in the entity name, lets impute the above name for those missing and entity name
data2 <- data[,`:=`(impute_entity_name = entity_name %>% map_chr(impute_entity_name))]


# create a separate table for only board members.
...
# create a table of only entity information without board member information.
entities <- data2[,.(
  entity_name,
...
entities2 <- entities %>% 
  ...
  .[,clean_entity_name := fcase(
    ...
    entity_name == "International Visitors Utah Council dba Utah Council for Citizen Diplomacy", "Utah Council for Citizen Diplomacy",
    ...
    entity_name == "St Anne's Center Inc.", "St Annes Center",
    ...
    entity_name == "The Family Connection Center, Inc. dba Open Doors Social Services", "Open Doors Social Services",
    ...
    # 
    # entity_name %>% stri_detect_regex(", Incorporated"), entity_name %>% stri_replace_all_regex(", Incorporated", ""),
    # entity_name %>% stri_detect_regex(", Inc\\."), entity_name %>% stri_replace_all_regex(", Inc\\.", ""),
    # entity_name %>% stri_detect_regex(", Inc"), entity_name %>% stri_replace_all_regex(", Inc", ""),
    # entity_name %>% stri_detect_regex(", INC"), entity_name %>% stri_replace_all_regex(", INC", ""),
    # entity_name %>% stri_detect_regex(" INC."), entity_name %>% stri_replace_all_regex(" INC.", ""),
    # entity_name %>% stri_detect_regex(", LLC"), entity_name %>% stri_replace_all_regex(", LLC", ""),
    # entity_name %>% stri_detect_regex(", LLP"), entity_name %>% stri_replace_all_regex(", LLP", ""),
    # entity_name %>% stri_detect_regex(" LLC"), entity_name %>% stri_replace_all_regex(" LLC", ""),
    # entity_name %>% stri_detect_regex(" LLP"), entity_name %>% stri_replace_all_regex(" LLP", ""),
    # entity_name %>% stri_detect_regex("Inc\\."), entity_name %>% stri_replace_all_regex("Inc\\.", ""),
    # entity_name %>% stri_detect_regex(" Inc$"), entity_name %>% stri_replace_all_regex(" Inc$",""),
...
.[sf_name != "salesforce.com"] %>% 
  ...
  .[,`:=`(new_name = fcase(
    ...
    sf_name %>% stri_detect_regex("Dept of"), sf_name %>% stri_replace_all_regex("Dept of","Department of"),
    
    #Weird "-" without proper spacing:
    ...
    sf_name %>% stri_detect_regex("Dist$"), sf_name %>% stri_replace_all_regex("Dist$.", "District"),
    ...
    sf_name %>% stri_detect_regex("@"), sf_name %>% stri_replace_all_regex("@", "at"),
    ...
    # Some names have multiple errors, the rest of this catches any extra errors:
...
# At some point I want to join the contact information, but lets get the
# entities done first.
...
# uncomment this bit of code to help flag those that need to be fixed.
...
entity_match <- sf_account2 %>% 
  ...
  .[,`:=`(new_entity_for_osa = is.na(sf_id),
          new_entity_for_lg = is.na(lg_id) & !(osa_govt_lvl %in% c("CPA Firm", "Health Provider"))
          )] %>% 
  .[,`:=`(name_cnt = .N), by=c("new_clean_name")] %>% 
  .[,.(
    ...
    new_entity_for_osa,
    new_entity_for_lg,
    name_changed_in_salesforce = new_clean_name!=old_sf_name,
    name_cnt,
    sf_id, 
    lg_id,
    primary_email, 
...
entity_match %>% 
  fwrite(paste0(Sys.Date(),"-entity-crosswalk.csv"))

# new entities for OSA count: 
entity_match[new_entity_for_osa==T] %>% nrow()
#new entities for LG count:
entity_match[new_entity_for_lg==T] %>% nrow()

# Duplicate Entities found: 
entity_match[name_cnt>1] %>% .[["new_clean_name"]] %>% unique() %>% length()
